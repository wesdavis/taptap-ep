...
export default function Settings() {
  ...
  useEffect(() => {
    async function checkOwnerStatus() {
        if (!user) return;
        const { data } = await supabase.from('locations').select('id').eq('owner_id', user.id).maybeSingle();
        if (data) setIsOwner(true);
    }
    checkOwnerStatus();
  }, [user]);

  // 🟢 YOUR ORIGINAL LOGOUT LOGIC (PRESERVED)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 pb-24">
      <div className="flex items-center gap-4 mb-8">
        <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="hover:bg-slate-800 text-slate-400 hover:text-white">
      ...
      <div className="space-y-6">
        {isOwner && (
            <div className="bg-gradient-to-r from-amber-950/40 to-slate-900 border border-amber-500/30 rounded-2xl p-2 shadow-xl animate-in slide-in-from-right-8 duration-500">
                <div onClick={() => navigate('/business')} className="flex items-center justify-between p-3 cursor-pointer hover:bg-white/5 rounded-xl transition-all group">
        ...
        <div className="bg-slate-900 rounded-2xl p-5 border border-slate-800 shadow-xl">
          <div className="flex items-center gap-2 mb-4 text-slate-400">
            ...
            <h2 className="font-bold text-xs uppercase tracking-wider">Support</h2>
          </div>
          <div className="space-y-1">
             <div className="px-2 py-2 flex justify-between items-center text-sm text-slate-500">
                <span>App Version</span><span className="font-mono text-xs">v1.2.1 (Beta)</span>
             </div>
             {/* 🟢 FIXED: CONTACT EMAILS */}
             <Button variant="ghost" className="w-full justify-start text-slate-300 hover:text-white hover:bg-slate-800 px-2 gap-2" onClick={() => window.location.href = 'mailto:wes@get-taptap.com?subject=Support Request'}>
                <Mail className="w-4 h-4" /> Contact Support
             </Button>
             <Button variant="ghost" className="w-full justify-start text-red-400 hover:text-red-300 hover:bg-red-900/10 px-2 gap-2" onClick={() => window.location.href = 'mailto:wes@get-taptap.com?subject=Bug Report'}>
